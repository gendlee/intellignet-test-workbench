...
const CASE_SPECS = [
  ...
  { txnCode: 'MISC9001', name: '查詢費率 — 已駁回', module: '費率查詢', req: REQ.loan, profile: 'pass', stateType: 'STATELESS', status: 'REJECTED', type: 'ExceptionHandling', testType: 'UAT' },
]
...
export function seedDB(db) {
  ...
  // 版本號（案例中心維護）：當前月起預生成 36 個月，每月集中（A）/ 非集中（Z）各一
  const versions = []
  const vNow = new Date()
  for (let i = 0; i < 36; i++) {
    ...
    versions.push(
      ...
      { code: key + 'Z', month: key, mode: 'Z', modeLabel: '非集中版本', createdAt: isoAt(daysAgo(i % 7, 9 + (i % 4))) },
    )
  }
