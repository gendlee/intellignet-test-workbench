...
| 頁面 | 地址 | 功能 |
|---|---|---|
| 儀表板 | `/index.html` | KPI、狀態環圖/模組條形/7 日趨勢、最近運行、待審核、批量運行橫幅 |
| 測試案例 | `/cases.html` | 交易碼搜索/多條件篩選、內聯最近運行結果（需求5）、勾選批量重跑（需求6）、批量 Word 導出、流量接入預留按鈕（需求3） |
| 案例錄入 | `/case-edit.html` | 主控 XML 輸入 → **AI 生成**新系統 HTTP/JSON 案例（需求1），可微調/重新生成；有狀態/無狀態標記 |
| 案例詳情 | `/case-detail.html` | Tab：本次結果（**欄位級 diff 高亮** + 合理性評估，需求4）/ 運行歷史 / 審核記錄；審核通過/駁回（需求2）；單條 Word 導出（需求12） |
| 壓力測試 | `/stress.html` | 計劃 CRUD（並發/時長/ramp-up）、啟動執行、TPS/延遲/錯誤率曲線 |
| 系統配置 | `/config.html` | URL 模板（固定段只讀 + 變數段可編輯）、默認請求頭、diff 比對規則、權限展示（需求9 預留位） |
| Diff 自測 | `/test-diff.html` | 瀏覽器內 diff 引擎自測面板 |

## 演示走查（建議路徑）

1. **Dashboard**：檢視 KPI 與三張圖表、最近運行、待審核清單
2. **錄入 + AI 生成**：`case-edit.html` → 貼入主控 XML → 「AI 生成新系統案例」→ 微調 → 保存
3. **審核**：`cases.html` 篩選「待審核」→ 進入詳情 → 審核通過/駁回
4. **執行 + diff**：詳情頁「執行」→ 檢視 verdict 與 diff 清單（新增/刪除/修改、可疑度、機器理由）→ 運行歷史回看任一運行
5. **批量回歸**：`cases.html` 勾選多條 → 「批量重跑」→ 進度抽屜逐案例結果
6. **導出**：單條或批量導出 Word（`.doc`，中文兼容）
7. **壓測**：`stress.html` → 新建計劃 → 啟動 → 曲線與 8 項 KPI
8. **配置**：`config.html` 調整 URL 模板/請求頭/diff 規則，再執行一次觀察 AI 生成與 diff 規則的影響
...
mock/                 # 僅 Node 端
  db.js               #   內存數據庫
  seed.js             #   種子：10 案例覆蓋全部 diff 場景 + 壓測計劃 + 配置
...
public/
  *.html              # 7 頁 + 404
  ...
  js/
    ...
    views/            #   diff-view（高亮清單）/ charts（手繪 SVG）/ word-export
    pages/            #   每頁一個入口模塊
```
...
v1.0.0（示範環境）— Mock 模式 · 零依賴。需求對照與實現細節見 `docs/API-LIST.md`。
